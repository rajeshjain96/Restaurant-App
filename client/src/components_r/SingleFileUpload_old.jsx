...
export default function SingleFileUpload(props) {
  let { action } = props;
  let { value } = props; // value is the fileName
  ...
  function getFileIndex() {
    for (let index = 0; index < props.singleFileList.length; index++) {
      let e = props.singleFileList[index];
          ...
          {action == "update" && !flagImageChange && (
            <>
              <img
                ...
                src={import.meta.env.VITE_API_URL + "/uploadedImages/" + value}
                alt=""
              />
            </>
          )}
