...
// GET /students/:id/grades/:gradeId
router.get("/enquiryFile/:id/:fileId", async (req, res) => {
  try {
    const enquiryId = ObjectId.createFromHexString(req.params.id);
    const fileId = ObjectId.createFromHexString(req.params.fileId);
    let result = await EnquiryService.getEnquiryFileById(enquiryId, fileId);
    // Find only the matching file from the array
